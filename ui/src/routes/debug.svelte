<script lang="ts">
    ...
    onDestroy(() => {
      sync.unsubscribe();
      ServerConnector.unsync(object);
    });


    function changeObject(obj:string){
      try{
        sync.unsubscribe();
        ServerConnector.unsync(object)
      }catch(e:any){
  ...
  <div style="width:100%; display:flex; flex-direction:column; background-color:black; overflow-y:scroll; height:100%;">
    <div style="flex-grow:0; flex-shrink:0; padding:0.5rem; text-align:center;">
      ...
      <li>
        <button on:click={()=>changeObject("crosspoint")}>
          Crosspoint
      </button>
      </li>
      ...
      <li>
        <button on:click={()=>changeObject("nmosConnectionState")}>
          NMOS Connection
      </button>
      </li>
  ...
  <div style="flex-grow:2;" class="debug-container">
